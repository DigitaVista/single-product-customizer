    ...
    class Sppcfw_Variation_Switcher
    {
        ...
        public function sppcfw_display_variation_switcher($html, $args)
        {
            if ($this->is_enabled() === 1) {
                ...
                $select = '<select id="' . esc_attr($attribute) . '" class="sppcfw-variation-select" data-attribute_name="' . esc_attr($attribute_name_field) . '" name="' . esc_attr($attribute_name_field) . '">';
                ...
                foreach ($options as $option) {
                    ...
                    switch ($attributes_type) {
                        case 'color':
                            $button .= '<button data-val="' . esc_attr($option_slug) . '" class="sppcfw-variation-button color" data-bg-color="' . esc_attr($option_meta) . '" type="button" data-attr="' . esc_attr($attribute) . '" title="' . esc_attr($option_label) . '"></button>';
                            break;

                        case 'icon':
                            $button .= '<button data-val="' . esc_attr($option_slug) . '" class="sppcfw-variation-button icon" type="button" data-attr="' . esc_attr($attribute) . '" title="' . esc_attr($option_label) . '">
                        ...
                        case 'image':
                            ...
                            $button .= '<button data-val="' . esc_attr($option_slug) . '" class="sppcfw-variation-button image" type="button" data-attr="' . esc_attr($attribute) . '" title="' . esc_attr($option_label) . '">
                                ' . $img . '
                            </button>';
                            break;
                        default:
                            $button .= '<button data-val="' . esc_attr($option_slug) . '" class="sppcfw-variation-button button" type="button" data-attr="' . esc_attr($attribute) . '">' . esc_html($option_label) . '</button>';
                    }
                }

                $select .= '</select>';

                return '<div class="sppcfw-cu-button-el" data-attribute="' . esc_attr($attribute) . '" data-is-taxonomy="' . ($is_taxonomy ? '1' : '0') . '">' . $select . $button . '</div>';
            }
